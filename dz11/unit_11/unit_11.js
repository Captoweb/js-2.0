...
// Task 6
// Напишите функцию f6, которая эмулирует работу метода push применительно к массиву d6. Т.е. добавляет значение из i-6 
// как последний элемент в массив d6. Эмуляция, значит, что мы получим результат аналогичный методу push без применения его. 
// Эмуляция заключается в присвоение значения как последнего элемента массива. Как вычислить индекс последнего элемента 
//используя длину массива - вы знаете из предыдущего урока. Повторный запуск функции должен также приводить к добавлению элемента.
...
function f6() {
    ...
    sum = sum + ' ' + val; // в новый массив засовываем элементы со старого + новый элемент
    //console.log(sum);
    d6 = d6 + ',' + val; // sum надо как то запихнуть в функцию, поэтому я перезаписываю d6
    sum = d6; 
    console.log(sum);
    //console.log(val);
    showArr('.out-6', d6);
