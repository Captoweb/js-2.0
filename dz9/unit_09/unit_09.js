...
function f1() {
    const wd = document.querySelector('.out-1');
    wd.style.width = '200px';
}
...
function f2() {
    const cls = document.querySelector('.out-2');
    cls.classList.add('bg-2');
}
...
function f3() {
    const cls = document.querySelector('.out-3');
    cls.classList.remove('bg-3');
}
...
function f4() {
    const cls = document.querySelector('.out-4');
    cls.classList.toggle('bg-4');
}
...
function f5() {
    const element = document.querySelector('.out-4');
    let test = document.querySelector('.out-5');

    if (element.classList.contains('bg-4')) {
        test.innerHTML = 'true';
        console.log('true');
    } else {
...
//  и добавляет ему класс bg-13. Также, созданному div добавляется событие onclick, по которому выполняется функция f13_1. Созданный div добавляется в out-13.
...
function f18() {
    ...
    console.log(p.getAttribute('data-b'));
    document.querySelector('.out-18').append(atr);
}
...
//  Task 19
// Добавьте кнопку .b-19, которая запускает функцию f19. Функция с помощью getAttribute получает data-b 
// атрибут с параграфов p-19 и выводит в out-19 через пробел. Обратите внимание, что элементов p-19 больше одного.
...
function f19() {
    ...
    for (let i = 0; i < p.length; i++) {
        output += p[i].getAttribute('data-b') + " ";
    }
...
function f20() {
    let out = document.querySelector('div.out-20');
    atr = out.setAttribute('title', 'go');
    console.log(out);
}
